...
def name_doc(document): #take a document.extension, return it's name without the extension
    for i in range(1,len(document)):
        if document[len(document)-i] == '.':
            return document[:len(document)-i]

def extension_doc(document): #take a document.extension, return it's extension without the name
    for i in range(1,len(document)):
        if document[len(document)-i] == '.':
            return document[len(document)-i+1:]

def refresh_database(): # load the database
    path = 'data/'
    list_doc = ['CEN.xlsx','Colis moyens par UM par circuit CEN.xlsx', 'Détail cde Frs OP blanc.csv','manif_2023.xlsx', 'PRépa CEN.csv', 'prev_recep_melusine.xlsx','ratios.xlsx','ex_engt r120 manif 632 2024_du211223.xlsx']
    database = path + 'melusine_database.db'
    engine = create_engine('sqlite:///' + database)
    for doc in list_doc:
        if extension_doc(doc)=='csv':
